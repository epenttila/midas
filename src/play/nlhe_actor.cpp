...
void nlhe_actor::act(nlhe_game& g)
{
    ...
    const nlhe_state_base::holdem_action action = state_->get_child(action_index)->get_action_index();
    ...
    const nlhe_state_base::holdem_action prev_action = prev_action_index != -1
        ? state_->get_child(prev_action_index)->get_action_index() : nlhe_state_base::INVALID_ACTION;

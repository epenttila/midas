...
struct hand_indexer_t {
    ...
    hand_indexer::hand_index_t round_size[MAX_ROUNDS];
    ...
    hand_indexer::hand_index_t * configuration_to_offset[MAX_ROUNDS];
};
...
double log(uint_fast32_t n)
{
    return log(static_cast<long double>(n));
}

double log(hand_indexer::hand_index_t n)
{
    return log(static_cast<long double>(n));
}
#endif
...
void enumerate_configurations_r(std::size_t rounds, const std::uint8_t cards_per_round[], 
    ...
    const Function& observe, void * data) {
        if (suit == SUITS) {
            ...
            bool was_equal = equal&1<<suit ? true : false;
            if (was_equal) {
...
template<class Function>
void enumerate_configurations(std::size_t rounds, const std::uint8_t cards_per_round[],
    const Function& observe, void * data) {
...
void count_configurations(uint_fast32_t round, uint_fast32_t /*configuration*/[], void * data) {
    uint_fast32_t * counts = reinterpret_cast<uint_fast32_t*>(data); ++counts[round];
}

void hand_indexer::tabulate_configurations(uint_fast32_t round, uint_fast32_t configuration[], void * data) {
    hand_indexer_t * indexer = reinterpret_cast<hand_indexer_t*>(data);
...
out:;
    ...
    for(uint_fast32_t i=0; i<SUITS;) {
        ...
        assert(size+SUITS-1 < hand_indexer::MAX_GROUP_INDEX);

        uint_fast32_t j=i+1; for(; j<SUITS && configuration[j] == configuration[i]; ++j) {} 
        for(uint_fast32_t k=i; k<j; ++k) {
            indexer->configuration_to_suit_size[round][id][k] = static_cast<uint_fast32_t>(size);
        }
...
void enumerate_permutations_r(std::size_t rounds, const std::uint8_t cards_per_round[], 
    uint_fast32_t round, uint_fast32_t remaining, 
...
void enumerate_permutations(std::size_t rounds, const std::uint8_t cards_per_round[],
    void (*observe)(uint_fast32_t, uint_fast32_t[], void*), void * data) {
...
hand_indexer::hand_index_t hand_indexer::hand_index_all(const uint8_t cards[], hand_index_t indices[]) const
{
    if (indexer->rounds) {
...
hand_indexer::hand_index_t hand_indexer::hand_index_last(const card_t cards[]) const
{
    hand_index_t indices[MAX_ROUNDS];
    return hand_index_all(cards, indices);
}

hand_indexer::hand_index_t hand_indexer::hand_index_next_round(const uint8_t cards[], hand_indexer_state_t * state) const
{
    ...
    for(uint_fast32_t i=0; i<indexer->cards_per_round[round]; ++i) {
        ...
        uint_fast32_t rank         = get_rank(cards[i]), suit = get_suit(cards[i]), rank_bit = 1<<rank;
        ranks[suit]               |= rank_bit;
    ...
    for(uint_fast32_t i=0; i<SUITS; ++i) {
        ...
        for(uint_fast32_t j=0; j<indexer->rounds; ++j) {
            ...
            for(uint_fast32_t k=0; k<n; ++k) {
                ...
                cards[location[j]++]       = get_card(card, i);
            }
            used |= rank_set;
        }
    }
...
hand_indexer::hand_indexer(const std::vector<std::uint8_t>& cards_per_round)
{
    ...
    std::fill(&nCr_ranks[0][0], &nCr_ranks[0][0] + (nCr_ranks.size() * nCr_ranks[0].size()), 0);
    ...
    std::fill(&nCr_groups[0][0], &nCr_groups[0][0] + (nCr_groups.size() * nCr_groups[0].size()), 0);
    ...
    std::fill(rank_set_to_index.begin(), rank_set_to_index.end(), 0);
    std::fill(&index_to_rank_set[0][0], &index_to_rank_set[0][0] + (index_to_rank_set.size() * index_to_rank_set[0].size()), 0);
    ...
    suit_permutations = reinterpret_cast<uint_fast32_t(*)[4]>(calloc(num_permutations, SUITS*sizeof(uint_fast32_t)));
    ...
    const auto rounds = cards_per_round.size();
    ...
    indexer = new hand_indexer_t;
    memset(indexer, 0, sizeof(hand_indexer_t));

    indexer->rounds = static_cast<decltype(indexer->rounds)>(rounds);
    memcpy(indexer->cards_per_round, cards_per_round.data(), rounds); 
    for(uint_fast32_t i=0, j=0; i<rounds; ++i) {
        indexer->round_start[i] = static_cast<uint8_t>(j); j += cards_per_round[i];
    }

    using namespace std::placeholders;

    memset(indexer->configurations, 0, sizeof(indexer->configurations));
    enumerate_configurations(rounds, cards_per_round.data(), count_configurations, indexer->configurations);

    for(uint_fast32_t i=0; i<rounds; ++i) {
        indexer->configuration_to_equal[i]     = reinterpret_cast<uint_fast32_t*>(calloc(indexer->configurations[i], sizeof(uint_fast32_t)));
        indexer->configuration_to_offset[i]    = reinterpret_cast<hand_index_t*>(calloc(indexer->configurations[i], sizeof(hand_index_t)));
        indexer->configuration[i]              = reinterpret_cast<uint_fast32_t(*)[4]>(calloc(indexer->configurations[i], SUITS*sizeof(uint_fast32_t)));
        indexer->configuration_to_suit_size[i] = reinterpret_cast<uint_fast32_t(*)[4]>(calloc(indexer->configurations[i], SUITS*sizeof(uint_fast32_t)));
    ...
    memset(indexer->configurations, 0, sizeof(indexer->configurations));
    enumerate_configurations(rounds, cards_per_round.data(), [this](uint_fast32_t rounds, uint_fast32_t* cfg, void* data)
    {
        tabulate_configurations(rounds, cfg, data);
    }, indexer);
    ...
    enumerate_permutations(rounds, cards_per_round.data(), count_permutations, indexer);

    for(uint_fast32_t i=0; i<rounds; ++i) {
        indexer->permutation_to_configuration[i] = reinterpret_cast<uint_fast32_t*>(calloc(indexer->permutations[i], sizeof(uint_fast32_t)));
        indexer->permutation_to_pi[i] = reinterpret_cast<uint_fast32_t*>(calloc(indexer->permutations[i], sizeof(uint_fast32_t)));
        if (!indexer->permutation_to_configuration || !indexer->permutation_to_pi) {
            return; 
        }
    }

    enumerate_permutations(rounds, cards_per_round.data(), tabulate_permutations, indexer);
}

std::size_t hand_indexer::get_size(int round) const
{
    assert(round < static_cast<int>(indexer->rounds));
...
int hand_indexer::get_rounds() const
{
    return indexer->rounds;
}
